...
- `📖 다이어리` 버튼 → 날짜 선택 팝업(월 달력). 날짜를 고르면 그 날의 다이어리를 본다.
...
- **📖 다이어리 버튼**: 날짜 선택 팝업(월 달력). 과거 날짜 선택 시 보드 읽기 전용 전환.
...
- 카드에는 그중 **최신(생성순) 5개까지만** 보여준다. 항목이 1개 이상이면 항상 "더보기" 버튼을 두고, 누르면 팝업으로 전체 목록(체크/수정/삭제 포함)을 볼 수 있다. 카드 자체가 세로로 길어지는 걸 막으면서 "한 화면에서 전체 파악" 원칙(2번)을 지키기 위함.
